...
export default class AnswerForm extends React.Component {
  ...
  handleClickPostAnswer() {
    ...
    if (emptyText || emptyUser || invalidUser) {
      ...
      axios.post("http://localhost:8000/newanswer", newAnswer).then((res) => {
        const insertId = res.data.insertId;
        const qaRelationship = [];
        qaRelationship.push(this.props.qid);
        qaRelationship.push(insertId);
        axios
          .post("http://localhost:8000/newqa", qaRelationship)
          .then((res) => {
            const error = {
              value: false,
              errors: "",
